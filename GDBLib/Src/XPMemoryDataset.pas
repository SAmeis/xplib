...
uses
  SysUtils, Classes, DB, Variants, Datasnap.DBClient, Datasnap.MidConst;

type
  TXPMemoryDataset = class(TClientDataset)
  private
...
uses
  Forms, Dialogs, DBConsts, Math, GDBHnd, SqlTimSt, Windows;

{$REGION 'LEGACY_CODE'}
{
  Antigo conteúdo da unit importada da JEDI
  ...
  ************************************************************************
  ...
  if (F.FieldKind = fkData) and (F.DataType in ftSupported - ftBlobTypes) then begin
  FIndexList.Add(F);
  end else begin
  ...
  try
  RecordToBuffer(Records[FRecordPos], PByte(TempBuffer)); //Cast para PByte forçado em XE6
  ...
  */
  ...
  /// <summary>
  /// Leitura do buffer do registro ativo
  /// </summary>
  /// <returns>Existe registro carregado</returns>
  ///   Modified by roger 30/09/2014 13:30:12 - Forçado typecast para TMDSRecBuffer a fim de acomodar o XE6
  begin
  case State of
  dsBrowse: begin
  ...
  RecBuf := TMDSRecBuffer(ActiveBuffer);
  end;
  end;
  ...
  dsCalcFields: begin
  RecBuf := TMDSRecBuffer(CalcBuffer);
  end;
  dsFilter: begin
  RecBuf := TMDSRecBuffer(TempBuffer);
  end;
  else begin
  RecBuf := nil;
  end;
  ...
  Data[0] := byte(Ord((Buffer <> nil) and not(VarIsNull(VarData) or VarIsEmpty(VarData))));
  ...
  else /* fkCalculated, fkLookup */ begin
  ...
  Result := (FRowsAffected = FRowsChanged) or ((FRowsAffected > 0) and (FRowsAffected <= FRowsChanged) and not FExactApply);
  ...
  begin
  Result := (Bookmark <> nil) and FActive and (TBookmarkData(Bookmark[0]) > low(Integer)) and (TBookmarkData(Bookmark[0]) <= FLastID);
  end;
  ...
  function TXPMemoryDataset.LoadFromDataSet(Source: TDataSet; RecordCount: Integer; Mode: TLoadMode;
