...
begin
  ...
  if (Self.LookupFields.Text <> '*') or (Self.LookupFields.Text = EmptyStr) then begin
  // Filtro para campos -> Valores originais sem tratamento
...
begin
  try
	...
	if (Self.LkpEdit.Text = EmptyStr) or (Pos(UpperCase(Self.LkpEdit.Text), UpperCase(Self.DBGrid.Columns[0].Field.AsString)) = 1)
	then begin
	  Self.FillingOK := TRUE;
	end else begin
