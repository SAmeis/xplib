...
{$ENDIF}
{$I INetLib.inc}
//Flags existentes neste módulo = WELL_FORMATED_HTML - identa a saida
//!!! Isto possibilita uma saida melhor formatada com maior consumo de CPU
{$DEFINE WELL_FORMATED_HTML }
//Eliminação de alertas de código legado
{$WARN IMPLICIT_STRING_CAST OFF}
//diretivas presentes em versão anterior
{ -$IFDEF IHTML4 }
...
type
	...
	THTMLTagEvent = procedure(Sender: TObject; Tag: THTMLTag; const TagString: string; TagParams: TStrings; var ReplaceText: string)
		of object;
	...
	{ HTML text styles }
	THTMLTextStyle = (tsNormal, tsBold, tsItalic, tsFixedPitch, tsBig, tsSmall, tsAddress, tsPreformat, tsEmphasised, tsStrong,
		tsDefinition, tsCode, tsSample, tsKeyboard, tsVariable, tsCitation, tsAbbreviation, tsAcronym, tsBlockQuote, tsQuote,
		tsSuperscript, tsSubscript, tsInserted, tsDeleted,
	...
	public
		constructor Create(sTitle: string; sBase: string = ''; sHeaderLanguage: string = '';
			diHeaderDirection: THTMLDirection = diDefault); virtual;
		destructor Destroy; override;
		function AsHTML: string; override; stdcall;
	...
	private
		...
		procedure SetOwnContents(bOwnContents: Boolean);
	public
		constructor Create(iRows, iCols: array of THTMLLength; sTagClass: string = ''; sId: string = '';
			sStyle: string = ''); virtual;
		destructor Destroy; override;
		function AsHTML: string; override; stdcall;
	...
	private
		...
		FAlignment: THTMLAlignmentHoriz;
	public
		constructor Create(iLevel: THTMLHeadingLevel; sText: string; sTagClass: string = ''; sId: string = '';
			sStyle: string = ''); virtual;
		function AsHTML: string; override; stdcall;
	...
	private
		...
		FText:      string;
	public
		constructor Create(tsStyle: THTMLTextStyle; sText: string; sTagClass: string = ''; sId: string = ''; sStyle: string = '');
			overload; virtual;
	...
	private
		...
		FIsMap:         Boolean;
	public
		constructor Create(sSource, sAlternateText: string; iHeight: THTMLLength = 0; iWidth: THTMLLength = 0;
			aiAlignment: THTMLAlignmentImage = aiDefault); virtual;
		function AsHTML: string; override; stdcall;
		property Source: string read FSource write FSource;
		property Height: THTMLLength read FHeight write FHeight;
		property Width: THTMLLength read FWidth write FWidth;
	...
	private
		...
		lstCoords:      TList;
	public
		constructor Create(shShape: THTMLShape; iCoords: array of THTMLPixels; sDestination, sAlternateText: string;
			sTarget: string = ''); virtual;
		destructor Destroy; override;
		function AsHTML: string; override; stdcall;
	...
	private
		...
		FUseMap:      string;
	public
		constructor Create(sClassId: string; sData: string = ''; sName: string = ''; ahAlignment: THTMLAlignmentHoriz = ahDefault;
			iHeight: THTMLLength = 0; iWidth: THTMLLength = 0); virtual;
		function AsHTML: string; override; stdcall;
	...
	public
		...
		constructor CreateUnordered(lbBulletType: THTMLListBullet = lbDefault; sTagClass: string = ''; sId: string = '';
			sStyle: string = '');
		constructor CreateOrdered(nsNumberScheme: THTMLNumberScheme = nsDefault; iStart: THTMLNumber = 0; sTagClass: string = '';
			sId: string = ''; sStyle: string = '');
		function AsHTML: string; override; stdcall;
	...
	private
		...
		FValue:        THTMLNumber;
	public
		constructor Create(liListItem: THTMLListItemType; sText: string; sTagClass: string = ''; sId: string = '';
			sStyle: string = ''); virtual;
		function AsHTML: string; override; stdcall;
	...
	public
		constructor Create(iWidth: THTMLLength = -100; { 100% } iBorderWidth: THTMLPixels = 2; sCaption: string = '';
			acCaptionAlignment: THTMLAlignmentCaption = acDefault); virtual;
		function AsHTML: string; override; stdcall;
	...
	private
		...
		FWidth: THTMLLength;
	public
		constructor Create(sStyle: string = ''; iSpan: THTMLNumber = 0; iWidth: THTMLLength = 0; sTagClass: string = '';
			sId: string = ''); virtual;
		function AsHTML: string; override; stdcall;
	...
	public
		constructor Create(sName, sImage: string; ahAlignment: THTMLAlignmentHoriz = ahDefault; iBorderWidth: THTMLPixels = 0);
			reintroduce;
	end;
	...
	private
		...
		procedure DoTagEvent(iTag: THTMLTag; const sTag: string; slsParams: TStrings; var sReplace: string);
...
const
	...
	sHTMLDirection: array [THTMLDirection] of string[3] = ('', 'ltr', 'rtl');
	sHTMLAlignmentHoriz: array [THTMLAlignmentHoriz] of string[6] = ('', 'left', 'center', 'right');
	sHTMLAlignmentVert: array [THTMLAlignmentVert] of string[6] = ('', 'top', 'middle', 'bottom');
	sHTMLAlignmentImage: array [THTMLAlignmentImage] of string[6] = ('', 'top', 'middle', 'bottom', 'left', 'right');
	sHTMLAlignmentCaption: array [THTMLAlignmentCaption] of string[6] = ('', 'top', 'bottom', 'left', 'right');
	sHTMLFrameScroll: array [THTMLFrameScroll] of string[3] = ('', 'yes', 'no');
	sHTMLShape: array [THTMLShape] of string[7]           = ('default', 'rect', 'circle', 'polygon');
	sHTMLParamTypes: array [THTMLParamTypes] of string[6] = ('data', 'ref', 'object');
	sHTMLListType: array [THTMLListType] of string[2]     = ('ul', 'ol', 'dl');
	sHTMLListBullet: array [THTMLListBullet] of string[6] = ('', 'disc', 'circle', 'square');
	sHTMLListItem: array [THTMLListItemType] of string[2] = ('li', 'dt', 'dd');
	sHTMLNumberScheme: array [THTMLNumberScheme] of string[1] = ('', 'A', 'a', 'I', 'i', '1');
	sHTMLClearTo: array [THTMLClearTo] of string[5]      = ('', 'left', 'right', 'all');
	sHTMLTextStyle: array [THTMLTextStyle] of string[10] = ('', 'b', 'i', 'tt', 'big', 'small', 'address', 'pre', 'em', 'strong',
		'dfn', 'code', 'samp', 'kbd', 'var', 'cite', 'abbr', 'acronym', 'blockquote', 'q', 'sup', 'sub', 'ins', 'del', 'u',
		'strike', 'blink', 'nobr', 'center');
	sHTMLTableFrame: array [THTMLTableFrame] of string[6] = ('void', 'above', 'below', 'hsides', 'vsides', 'lhs', 'rhs', 'box',
		'border');
	sHTMLTableRule: array [THTMLTableRule] of string[6] = ('none', 'groups', 'rows', 'cols', 'all');
	sHTMLTableAlignHoriz: array [THTMLTableAlignHoriz] of string[7] = ('', 'left', 'center', 'right', 'justify', 'char');
	sHTMLTableAlignVert: array [THTMLTableAlignVert] of string[8] = ('', 'top', 'middle', 'bottom', 'baseline');
	sHTMLTableRowGrouping: array [THTMLTableRowGrouping] of string[5] = ('thead', 'tbody', 'tfoot');
	sHTMLFormMethod: array [THTMLFormMethod] of string[4] = ('get', 'post');
	sHTMLFieldType: array [THTMLFieldType] of string[8]   = ('text', 'password', 'checkbox', 'radio', 'submit', 'reset', 'button',
		'image', 'hidden', 'file');
...
begin
	...
	for i := 1 to Length(sText) do
		case sText[i] of
			...
			'À':
				Result := Result + '&Agrave;'; { 192, #C0 }
			'Á':
				Result := Result + '&Aacute;'; { 193, #C1 }
			'Â':
				Result := Result + '&Acirc;'; { 194, #C2 }
			'Ã':
				Result := Result + '&Atilde;'; { 195, #C3 }
			'Ä':
				Result := Result + '&Auml;'; { 196, #C4 }
			'Å':
				Result := Result + '&Aring;'; { 197, #C5 }
			'Æ':
				Result := Result + '&AElig;'; { 198, #C6 }
			'Ç':
				Result := Result + '&Ccedil;'; { 199, #C7 }
			'È':
				Result := Result + '&Egrave;'; { 200, #C8 }
			'É':
				Result := Result + '&Eacute;'; { 201, #C9 }
			'Ê':
				Result := Result + '&Ecirc;'; { 202, #CA }
			'Ë':
				Result := Result + '&Euml;'; { 203, #CB }
			'Ì':
				Result := Result + '&Igrave;'; { 204, #CC }
			'Í':
				Result := Result + '&Iacute;'; { 205, #CD }
			'Î':
				Result := Result + '&Icirc;'; { 206, #CE }
			'Ï':
				Result := Result + '&Iuml;'; { 207, #CF }
			'Ð':
				Result := Result + '&ETH;'; { 208, #D0 }
			'Ñ':
				Result := Result + '&Ntilde;'; { 209, #D1 }
			'Ò':
				Result := Result + '&Ograve;'; { 210, #D2 }
			'Ó':
				Result := Result + '&Oacute;'; { 211, #D3 }
			'Ô':
				Result := Result + '&Ocirc;'; { 212, #D4 }
			'Õ':
				Result := Result + '&Otilde;'; { 213, #D5 }
			'Ö':
				Result := Result + '&Ouml;'; { 214, #D6 }
			'×':
				Result := Result + '&times;'; { 215, #D7 }
			'Ø':
				Result := Result + '&Oslash;'; { 216, #D8 }
			'Ù':
				Result := Result + '&Ugrave;'; { 217, #D9 }
			'Ú':
				Result := Result + '&Uacute;'; { 218, #DA }
			'Û':
				Result := Result + '&Ucirc;'; { 219, #DB }
			'Ü':
				Result := Result + '&Uuml;'; { 220, #DC }
			'Ý':
				Result := Result + '&Yacute;'; { 221, #DD }
			'Þ':
				Result := Result + '&THORN;'; { 222, #DE }
			'ß':
				Result := Result + '&szlig;'; { 223, #DF }
			'à':
				Result := Result + '&agrave;'; { 224, #E0 }
			'á':
				Result := Result + '&aacute;'; { 225, #E1 }
			'â':
				Result := Result + '&acirc;'; { 226, #E2 }
			'ã':
				Result := Result + '&atilde;'; { 227, #E3 }
			'ä':
				Result := Result + '&auml;'; { 228, #E4 }
			'å':
				Result := Result + '&aring;'; { 229, #E5 }
			'æ':
				Result := Result + '&aelig;'; { 230, #E6 }
			'ç':
				Result := Result + '&ccedil;'; { 231, #E7 }
			'è':
				Result := Result + '&egrave;'; { 232, #E8 }
			'é':
				Result := Result + '&eacute;'; { 233, #E9 }
			'ê':
				Result := Result + '&ecirc;'; { 234, #EA }
			'ë':
				Result := Result + '&euml;'; { 235, #EB }
			'ì':
				Result := Result + '&igrave;'; { 236, #EC }
			'í':
				Result := Result + '&iacute;'; { 237, #ED }
			'î':
				Result := Result + '&icirc;'; { 238, #EE }
			'ï':
				Result := Result + '&iuml;'; { 239, #EF }
			'ð':
				Result := Result + '&eth;'; { 240, #F0 }
			'ñ':
				Result := Result + '&ntilde;'; { 241, #F1 }
			'ò':
				Result := Result + '&ograve;'; { 242, #F2 }
			'ó':
				Result := Result + '&oacute;'; { 243, #F3 }
			'ô':
				Result := Result + '&ocirc;'; { 244, #F4 }
			'õ':
				Result := Result + '&otilde;'; { 245, #F5 }
			'ö':
				Result := Result + '&ouml;'; { 246, #F6 }
			'÷':
				Result := Result + '&divide;'; { 247, #F7 }
			'ø':
				Result := Result + '&oslash;'; { 248, #F8 }
			'ù':
				Result := Result + '&ugrave;'; { 249, #F9 }
			'ú':
				Result := Result + '&uacute;'; { 250, #FA }
			'û':
				Result := Result + '&ucirc;'; { 251, #FB }
			'ü':
				Result := Result + '&uuml;'; { 252, #FC }
			'ý':
				Result := Result + '&yacute;'; { 253, #FD }
			'þ':
				Result := Result + '&thorn;'; { 254, #FE }
			'ÿ':
				Result := Result + '&yuml;'; { 255, #FF }
		else
			Result := Result + sText[i];
		end;
end;
...
begin
	Result := TextAttribute('style', Style) + TextAttribute('id', Id) + TextAttribute('class', TagClass) +
		TextAttribute('lang', Language) + TextAttribute('title', Title) + NumericAttribute('tabindex', TabIndex);
	if AccessKey <> #0 then
...
{ Initialise }
constructor THTMLDocument.Create(sTitle: string; sBase: string = ''; sHeaderLanguage: string = '';
	diHeaderDirection: THTMLDirection = diDefault);
begin
	inherited Create;
...
begin
	...
	if (heStrictHTML4 in HTMLErrorLevel) and ((BackgroundImage <> '') or (BackgroundColour <> clNone) or (TextColour <> clNone) or
		(LinkColour <> clNone) or (VisitedColour <> clNone) or (ActiveColour <> clNone)) then
	...
	Result := Result + '>'#13#10'<title>' + EncodeSpecialChars(Title) + '</title>'#13#10;
	if Base <> '' then
	...
	if FrameSet <> nil then { Body is a frameset }
		...
		Result := Result + '<body' + TextAttribute('background', BackgroundImage) + ColourAttribute('bgcolor', BackgroundColour) +
			ColourAttribute('text', TextColour) + ColourAttribute('link', LinkColour) + ColourAttribute('vlink', VisitedColour) +
			ColourAttribute('alink', ActiveColour) + BaseAttributesAsHTML + '>'#13#10 + ContentsAsHTML + '</body>'#13#10;
	Result := Result + '</html>';
end;
...
begin
	...
	{ Generate HTML }
	Result := '<meta' + TextAttribute('name', name) + TextAttribute('http-equiv', HTTPEquivalent) +
...
begin
	...
	{ Generate HTML }
	Result := '<link' + TextAttribute('href', Destination) + TextAttribute('rel', RelType) + TextAttribute('rev', RevType) +
		TextAttribute('target', Target) + TextAttribute('type', ContentType) + TextAttribute('title', Title) + BaseAttributesAsHTML
		+ '>'#13#10;
end;
...
var
	i: Integer;
begin
	Result := '<style' + TextAttribute('type', StyleType) + TextAttribute('media', Media) + TextAttribute('title', Title) +
		BaseAttributesAsHTML + '>'#13#10;
	if CommentOut then
		Result := Result + '<!--'#13#10;
	for i := 0 to slsStyles.Count - 1 do
		Result := Result + slsStyles.Names[i] + ' { ' + slsStyles.Values[slsStyles.Names[i]] + ' }'#13#10;
	if CommentOut then
		Result := Result + '-->'#13#10;
...
begin
	Result := '<frameset' + TextAttribute('rows', RowsAsText) + TextAttribute('cols', ColsAsText) + BaseAttributesAsHTML + '>'#13#10
		+ ContentsAsHTML;
	if FNoFrames.Count > 0 then
		Result := Result + '<noframes>'#13#10 + FNoFrames.ContentsAsHTML + '</noframes>'#13#10;
...
begin
	if RowOrCol = iRow then
		...
		lst := FCols;
	Result := '';
	if (lst.Count > 0) and (THTMLLength(lst[0]) <> 0) then begin
...
begin
	...
	{ Generate HTML }
	Result := '<frame' + TextAttribute('src', Source) + TextAttribute('target', Target) + TextAttribute('name', name) +
		BooleanAttribute(not FrameBorder, 'frameborder=0') + NumericAttribute('marginwidth', MarginWidth) +
		NumericAttribute('marginheight', MarginHeight) + BooleanAttribute(NoResize, 'noresize');
...
begin
	...
	{ Generate HTML }
	Result := '<iframe' + TextAttribute('src', Source) + TextAttribute('target', Target) + TextAttribute('name', name) +
		BooleanAttribute(not FrameBorder, 'frameborder=0') + NumericAttribute('marginwidth', MarginWidth) +
		NumericAttribute('marginheight', MarginHeight) + NumericAttribute('width', Width) + NumericAttribute('height', Height) +
		AlignCaptionAttribute('align', Alignment);
	if Scrolling <> fsAuto then
		Result := Result + ' scrolling=' + sHTMLFrameScroll[Scrolling];
	Result := Result + BaseAttributesAsHTML + '>'#13#10 + FNoFrames.ContentsAsHTML + '</iframe>'#13#10;
end;
...
begin
	if FScriptType <> sScriptType then begin
...
begin
	...
	{ Generate HTML }
	Result := '<script' + TextAttribute('type', ScriptType) + TextAttribute('src', Source) + BooleanAttribute(Defer, 'defer') +
		'>'#13#10;
	if Code <> '' then begin
		...
		Result := Result + Code;
		if HideInComment then
			Result := Result + #13#10 + CommentSequence + ' ' + sHideScriptEnd + ' -->'#13#10;
	end;
...
begin
	Result := '<noscript' + BaseAttributesAsHTML + '>'#13#10 + ContentsAsHTML + '</noscript>'#13#10;
end;
...
const
	sDivisionType: array [Boolean, 1 .. 2] of string[4] = (('span', ''), ('div', #13#10));
begin
	{ Check for errors }
	...
	{ Generate HTML }
	Result := '<' + sDivisionType[BlockLevel, 1] + BaseAttributesAsHTML + AlignHorizAttribute('align', Alignment) + '>' +
		sDivisionType[BlockLevel, 2] + EncodeSpecialChars(Text) + ContentsAsHTML + '</' + sDivisionType[BlockLevel, 1] + '>' +
		sDivisionType[BlockLevel, 2];
end;
...
begin
	...
	{ Generate HTML }
	Result := '<p' + AlignHorizAttribute('align', Alignment) + BaseAttributesAsHTML + '>' + EncodeSpecialChars(Text) +
...
{ Initialise }
constructor THTMLHeading.Create(iLevel: THTMLHeadingLevel; sText: string; sTagClass: string = ''; sId: string = '';
	sStyle: string = '');
begin
	inherited Create;
	FLevel := iLevel;
...
begin
	...
	{ Generate HTML }
	Result := '<h' + IntToStr(Level) + AlignHorizAttribute('align', Alignment) + BaseAttributesAsHTML + '>' +
		EncodeSpecialChars(Text) + ContentsAsHTML + '</h' + IntToStr(Level) + '>'#13#10;
end;
...
{ Initialise }
constructor THTMLImage.Create(sSource, sAlternateText: string; iHeight: THTMLLength = 0; iWidth: THTMLLength = 0;
	aiAlignment: THTMLAlignmentImage = aiDefault);
begin
	inherited Create;
	FSource := sSource;
...
begin
	...
	{ Generate HTML }
	Result := '<img src="' + Source + '" border=' + IntToStr(BorderWidth) + TextAttribute('alt', AlternateText) +
		AlignImageAttribute('align', Alignment) + NumericAttribute('width', Width) + NumericAttribute('height', Height) +
		NumericAttribute('hspace', HorizSpace) + NumericAttribute('vspace', VertSpace) + TextAttribute('usemap', UseMap) +
		BooleanAttribute(IsMap, 'ismap') + BaseAttributesAsHTML + '>';
end;
...
begin
	...
	{ Generate HTML }
	Result := #13#10'<map name="' + name + '"' + BaseAttributesAsHTML + '>'#13#10 + ContentsAsHTML + '</map>'#13#10;
end;

{ THTMLImageMapArea ----------------------------------------------------------- }

{ Initialise }
constructor THTMLImageMapArea.Create(shShape: THTMLShape; iCoords: array of THTMLPixels; sDestination, sAlternateText: string;
	sTarget: string = '');
begin
	inherited Create;
...
procedure THTMLImageMapArea.SetShape(shShape: THTMLShape; iCoords: array of THTMLPixels);
var
	i, iCount: Integer;
begin
	{ Check for errors }
	iCount := high(iCoords) + 1;
	if (heErrors in HTMLErrorLevel) and (((shShape = shRect) and (iCount <> 4)) or ((shShape = shCircle) and (iCount <> 3)) or
...
begin
	...
	if Shape <> shDefault then begin
		sSep := ' coords="';
		for i := 0 to lstCoords.Count - 1 do begin
			Result := Result + sSep + IntToStr(LongInt(lstCoords[i]));
			sSep := ',';
		end;
		Result := Result + '"';
	end;
	Result := Result + BooleanAttribute((Destination = ''), 'nohref') + TextAttribute('href', Destination) +
		TextAttribute('target', Target) + TextAttribute('alt', AlternateText) + BaseAttributesAsHTML + '>'#13#10;
end;

{ THTMLObject ----------------------------------------------------------------- }

{ Initialise }
constructor THTMLObject.Create(sClassId: string; sData: string = ''; sName: string = '';
	ahAlignment: THTMLAlignmentHoriz = ahDefault; iHeight: THTMLLength = 0; iWidth: THTMLLength = 0);
begin
	inherited Create;
	FClassId := sClassId;
	FData := sData;
...
begin
	...
	{ Generate HTML }
	Result := '<object' + TextAttribute('classid', ClassId) + TextAttribute('codebase', CodeBase) +
		TextAttribute('codetype', CodeType) + TextAttribute('data', Data) + TextAttribute('type', DataType) +
		TextAttribute('name', name) + TextAttribute('standby', StandBy) + NumericAttribute('width', Width) +
		NumericAttribute('height', Height) + AlignHorizAttribute('align', Alignment) + NumericAttribute('border', BorderWidth) +
		NumericAttribute('hspace', HorizSpace) + NumericAttribute('vspace', VertSpace) + TextAttribute('usemap', UseMap) +
		BaseAttributesAsHTML + '>'#13#10 + ContentsAsHTML + '</object>'#13#10;
end;
...
begin
	...
	{ Generate HTML }
	Result := '<param' + TextAttribute('name', name) + TextAttribute('value', Value) +
		TextAttribute('valuetype', sHTMLParamTypes[ValueType]) + TextAttribute('type', ContentType) + BaseAttributesAsHTML +
		'>'#13#10;
end;
...
constructor THTMLList.CreateUnordered(lbBulletType: THTMLListBullet = lbDefault; sTagClass: string = ''; sId: string = '';
	sStyle: string = '');
begin
	//FDEF Delphi 4
...
constructor THTMLList.CreateOrdered(nsNumberScheme: THTMLNumberScheme = nsDefault; iStart: THTMLNumber = 0; sTagClass: string = '';
	sId: string = ''; sStyle: string = '');
begin
	//IFDEF Delphi 4
...
begin
	{ Check for errors }
	if (heStrictHTML4 in HTMLErrorLevel) and ((NumberScheme <> nsDefault) or (Start <> 0) or (BulletType <> lbDefault)) then
	...
	if ListType = ltOrdered then
		Result := Result + BooleanAttribute((NumberScheme <> nsDefault), 'type=' + sHTMLNumberScheme[NumberScheme]) +
			NumericAttribute('start', Start)
	else if ListType = ltUnordered then begin
		if BulletType <> lbDefault then
			Result := Result + ' type=' + sHTMLListBullet[BulletType];
	end;
	Result := Result + BaseAttributesAsHTML + '>'#13#10 + ContentsAsHTML + '</' + sHTMLListType[ListType] + '>'#13#10;
end;

{ THTMLListItem --------------------------------------------------------------- }

{ Initialise }
constructor THTMLListItem.Create(liListItem: THTMLListItemType; sText: string; sTagClass: string = ''; sId: string = '';
	sStyle: string = '');
begin
	inherited Create;
...
begin
	{ Check for errors }
	if (heStrictHTML4 in HTMLErrorLevel) and ((NumberScheme <> nsDefault) or (Value <> 0)) then
		raise EHTMLError.Create(Format(sDeprecatedAttribute, [sHTMLListItem[ListItem]]));

	{ Generate HTML }
	Result := '<' + sHTMLListItem[ListItem] + BooleanAttribute((NumberScheme <> nsDefault),
		'type=' + sHTMLNumberScheme[NumberScheme]) + NumericAttribute('value', Value) + BaseAttributesAsHTML + '>' +
		EncodeSpecialChars(Text) + ContentsAsHTML + '</' + sHTMLListItem[ListItem] + '>'#13#10;
end;
...
begin
	...
	{ Generate HTML }
	Result := '<a' + TextAttribute('href', Destination) + TextAttribute('name', name) + TextAttribute('target', Target) +
		TextAttribute('type', ContentType) + BaseAttributesAsHTML + '>' + EncodeSpecialChars(Text) + ContentsAsHTML + '</a>';
end;
...
begin
	{ Check for errors }
	if (heStrictHTML4 in HTMLErrorLevel) and ((Alignment <> ahDefault) or NoShade or (Width <> 0) or (Size <> 0)) then
		raise EHTMLError.Create(Format(sDeprecatedAttribute, ['hr']));

	{ Generate HTML }
	Result := '<hr' + NumericAttribute('size', Size) + NumericAttribute('width', Width) + AlignHorizAttribute('align', Alignment) +
...
begin
	Result := '<br' + ClearAttribute('clear', ClearTo) + BaseAttributesAsHTML + '>'#13#10;
end;
...
begin
	...
	if CharInSet(AlignToChar, [#32 .. #255]) then
...
begin
	{ Check for errors }
	if (heStrictHTML4 in HTMLErrorLevel) and ((Background <> clNone) or (BorderColour <> clNone) or (BorderHighlight <> clNone) or
		(BorderShadow <> clNone)) then begin
...
{ Initialise }
constructor THTMLTable.Create(iWidth: THTMLLength = -100; { 100% } iBorderWidth: THTMLPixels = 2; sCaption: string = '';
	acCaptionAlignment: THTMLAlignmentCaption = acDefault);
begin
	inherited Create;
	FWidth := iWidth;
...
begin
	...
	{ Generate HTML }
	Result := '<table border=' + IntToStr(BorderWidth) + NumericAttribute('width', Width) + TextAttribute('summary', Summary);
	...
	Result := Result + NumericAttribute('cellspacing', CellSpacing) + NumericAttribute('cellpadding', CellPadding) + ColoursAsHTML +
		BaseAttributesAsHTML + '>'#13#10;
	{ Include caption tag if present }
	if Caption <> '' then
		Result := Result + '<caption' + AlignCaptionAttribute('align', CaptionAlignment) + '>' + EncodeSpecialChars(Caption) +
			'</caption>'#13#10;
...
begin
	Result := '<' + sHTMLTableRowGrouping[RowGrouping] + AlignmentsAsHTML + BaseAttributesAsHTML + '>'#13#10 + ContentsAsHTML + '</'
		+ sHTMLTableRowGrouping[RowGrouping] + '>'#13#10;
end;

{ THTMLTableColumnGroup ------------------------------------------------------- }

{ Initialise }
constructor THTMLTableColumnGroup.Create(sStyle: string = ''; iSpan: THTMLNumber = 0; iWidth: THTMLLength = 0;
	sTagClass: string = ''; sId: string = '');
begin
	inherited Create;
	Style := sStyle;
	FSpan := iSpan;
...
begin
	Result := '<colgroup' + NumericAttribute('span', Span) + NumericAttribute('width', Width) + AlignmentsAsHTML +
...
{ Initialise }
constructor THTMLTableColumn.Create(sStyle: string = ''; iSpan: THTMLNumber = 0; iWidth: THTMLLength = 0; sTagClass: string = '';
	sId: string = '');
begin
	inherited Create;
	Style := sStyle;
...
begin
	Result := '<col' + NumericAttribute('span', Span) + NumericAttribute('width', Width);
	if AlignHoriz <> thDefault then
...
begin
	...
	Result := BooleanAttribute(ColSpan <> 1, 'colspan=' + IntToStr(ColSpan)) +
		BooleanAttribute(RowSpan <> 1, 'rowspan=' + IntToStr(RowSpan)) + NumericAttribute('width', Width) +
		NumericAttribute('height', Height);
end;
...
begin
	Result := '<th' + SpansAsHTML + AlignmentsAsHTML + ColoursAsHTML + BaseAttributesAsHTML + '>' + EncodeSpecialChars(Text) +
		ContentsAsHTML + '</th>';
end;
...
//Return formatted HTML
begin
	Result := '<td' + SpansAsHTML + AlignmentsAsHTML + ColoursAsHTML + BaseAttributesAsHTML + '>' + EncodeSpecialChars(Text) +
		ContentsAsHTML + '</td>';
end;
...
begin
	...
	{ Generate HTML }
	Result := '<form method=' + sHTMLFormMethod[Method] + ' action="' + Destination + '"' + TextAttribute('target', Target) +
...
begin
	...
	if Legend <> '' then
		Result := Result + '<legend' + AlignCaptionAttribute('align', LegendAlignment) + '>' + EncodeSpecialChars(Legend) +
			'</legend>'#13#10;
...
begin
	...
	{ Generate HTML }
	Result := '<input type=' + sHTMLFieldType[FieldType] + TextAttribute('name', name) + TextAttribute('value', Value) +
		BooleanAttribute(Checked, 'checked') + BooleanAttribute(Disabled, 'disabled') + BooleanAttribute(readonly, 'readonly') +
		NumericAttribute('size', Size) + NumericAttribute('maxlength', MaxLength) + TextAttribute('src', Image) +
		AlignHorizAttribute('align', Alignment) + NumericAttribute('border', BorderWidth) + TextAttribute('onclick', OnClick) +
	//Onclick event
...
{ Initialise }
constructor THTMLImageField.Create(sName, sImage: string; ahAlignment: THTMLAlignmentHoriz = ahDefault;
	iBorderWidth: THTMLPixels = 0);
begin
...
begin
	Result := '<button type=' + sHTMLFieldType[ButtonType] + TextAttribute('name', name) + TextAttribute('value', Value) +
		BooleanAttribute(Disabled, 'disabled') + BaseAttributesAsHTML + '>' + EncodeSpecialChars(Text) + ContentsAsHTML +
		'</button>';
end;
...
begin
	...
	{ Generate HTML }
	Result := '<select' + TextAttribute('name', name) + NumericAttribute('size', Size) + BooleanAttribute(Multiple, 'multiple') +
		BooleanAttribute(Disabled, 'disabled') + BaseAttributesAsHTML + '>' + ContentsAsHTML + '</select>';
end;
...
begin
	...
	{ Generate HTML }
	Result := '<optgroup' + TextAttribute('label', GroupLabel) + BooleanAttribute(Disabled, 'disabled') + BaseAttributesAsHTML + '>'
...
{ Initialise }
constructor THTMLSelectOption.Create(sOption: string; sValue: string = ''; bSelected: Boolean = False);
begin
	inherited Create;
	FOption := sOption;
	FValue := sValue;
...
begin
	Result := '<option' + TextAttribute('value', Value) + BooleanAttribute(Selected, 'selected') +
		BooleanAttribute(Disabled, 'disabled') + TextAttribute('label', ShortLabel) + BaseAttributesAsHTML + '>' +
...
begin
	...
	{ Generate HTML }
	Result := '<textarea' + TextAttribute('name', name) + NumericAttribute('cols', Cols) + NumericAttribute('rows', Rows) +
		BooleanAttribute(Disabled, 'disabled') + BooleanAttribute(readonly, 'readonly') + BaseAttributesAsHTML + '>' +
...
begin
	Result := '<label' + TextAttribute('for', ForId) + BaseAttributesAsHTML + '>' + EncodeSpecialChars(Text) + ContentsAsHTML +
		'</label>';
end;
...
var
	...
	begin
		Result := '';
		while (iPos <= Length(sAttributes)) and not(CharInSet(sAttributes[iPos], cDelims)) do begin
...
begin
	...
	while True do begin
		...
		case sAttributes[iPos] of
			...
			'"', '''': { Value delimited by quotes } begin
...
procedure THTMLStream.DoTagEvent(iTag: THTMLTag; const sTag: string; slsParams: TStrings; var sReplace: string);
begin
...
begin
	Result := '';
	if LowerCase(sTag) = sTimeSubstitute then begin
