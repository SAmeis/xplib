...
type
   ...
   THTMLLookupValues = procedure( Sender : Tobject; Field : TField; ValueList : TStrings; Cell: THTMLTableDetail; UseValues : boolean ) of object;
   TOnGetIntValue  =   procedure( Sender : TObject; var Value : longint );
   TWebDataSetInfoStorage  =   class;
   TOnWebGridSeekError =   procedure( WebDataSetInfoStorage : TWebDataSetInfoStorage; Index : integer;
                                      Request : TWebRequest; Response : TWebResponse; var Proceed : boolean ) of object;
   TOnCheckedRecord    =   function ( DataSet : TDataSet; Request : TWebRequest; Response : TWebResponse;
                                       Index : integer; var Checked : boolean ) : boolean of object;
	//Referencia posterior
	TDataSetWebGrid = class;
   TWebUpdateMode      =   ( wumInsert, wumEdit, wumDelete );
   TWebUpdateModes     =   set of TWebUpdateMode;
   TWebUpdateRecordProc    =   function ( Sender : TWebDataSetInfoStorage; RecordIndex : integer; Request : TWebRequest; Response : TWebResponse ) : boolean of object;
   //propriedades individual de cada WebDataSet
	...
	protected
       ...
       function  DoDeleteCheckedRecords( DS: TDataSet; Request: TWebRequest; Response: TWebResponse; Log : TWebDataSetUpdate): integer;
	...
	published //Estes campos sao published apenas para efeito de streaming
       property AppendRecordsAvailble : integer read FAppendRecordsAvailble write FAppendRecordsAvailble; //espaco para novos registros
       ...
       property ReadOnly : boolean read FReadOnly write FReadOnly; //Evita alteracoes nos dados diretamente, apenas pelas enumeracoes
       property RecordOffSet : longint read FRecordOffSet write FRecordOffSet; //Property usada apenas qdo nao ha KeyFields eh basada em RecNo
       ...
       property WebUpdateModes : TWebUpdateModes read FWebUpdateMode write FWebUpdateMode;  //Indica operacoes permitidas durante a atualizacao dos dados
	end;
   ...
   public
       ...
       property OnPostError : TDataSetErrorEvent read FOnPostError write FOnPostError;
       property OnNewRecord : TDataSetNotifyEvent read FOnNewRecord write FOnNewRecord;
       property OnNextWebDataSetInfoStorage : TOnNextWebDataSetInfoStorage read FOnNextWebDataSetInfoStorage write FOnNextWebDataSetInfoStorage;
       property OnWebGridSeekError : TOnWebGridSeekError read FOnWebGridSeekError write FOnWebGridSeekError; //Override Childs
       property WebUpdateModes : TWebUpdateModes read FWebUpdateMode write FWebUpdateMode;  //Indica operacoes permitidas durante a atualizacao dos dados
       property ReadOnly : boolean read FReadOnly write FReadOnly; //Evita a chamada aos metodos que alteram valores, mas chama enumeradores
...
//----------------------------------------------------------------------------------------------------------------------
	...
	public
       property AppendRecordsBackground : TColor read FAppendRecordsBackground write FAppendRecordsBackground;
		property CheckRowPosition : THTMLCheckRowPosition read FCheckRowPosition
                write FCheckRowPosition default hcrPosNone; //Coluna para assinalar pertinencia a um grupo de selecao
       ...
       property OnGetColumnCheckBox : THTMLCheckBoxEvent read FOnGetColumnCheckBox write FOnGetColumnCheckBox; //Qdo coluna inclusao gerada
...
uses
   dbTables, Str_Pas, IOObj, Variants;
...
//----------------------------------------------------------------------------------------------------------------------
begin
   if Self.FWebAppendedRecordsAvailable <= 0 then begin //Se nao houver geracao de registros para insercao checa registros de outro tipo
       inherited;
   end;
end;
...
begin
   ...
   if FKeyFieldsValues.Count > 0 then begin //Existem valores chaves para serem confrontados
       ...
       for FieldCnt:=0 to ManyKeys-1 do begin
           if DS.FieldByName( Self.FKeyFieldsNames[ FieldCnt ] ).AsString <>
              Self.FKeyFieldsValues.Strings[ ManyKeys*Index + FieldCnt ] then
           begin
               Result:=False;
               Exit;
           end;
...
function TWebDataSetInfoStorage.DoAddNewRecords(DS: TDataSet; Request: TWebRequest; Response: TWebResponse; Log : TWebDataSetUpdate): integer;
//----------------------------------------------------------------------------------------------------------------------
...
function TWebDataSetInfoStorage.DoDeleteCheckedRecords(DS: TDataSet; Request: TWebRequest; Response: TWebResponse; Log : TWebDataSetUpdate): integer;
//----------------------------------------------------------------------------------------------------------------------
...
begin
   ...
   if (Self.FKeyFieldsValues.Count >= MaxIndex) and (Self.FKeyFieldsNames.Count > 0) then begin //Items para pesquisa
       ...
       if DS is TTable then begin
           ...
           try
               TTable(DS).IndexName:=EmptyStr;   //!!!! Colocar na documentacao que os valores para a chave SEMPRE devem ser primarios
               Result:=TTable(DS).Locate( KeyFields, KeyValues, [loPartialKey] ); //Chave parcial pode permancer chamando CheckCursorPos
           finally
...
function TWebDataSetInfoStorage.DoUpdateDataSetValues( DS: TDataSet; Request: TWebRequest; Response: TWebResponse; Log : TWebDataSetUpdate): integer;
//----------------------------------------------------------------------------------------------------------------------
	...
	for i:=Self.FRecordsLoaded-Self.FAppendRecordsAvailble-1 downto 0 do begin
		if Self.DoSeekDataSet( DS, i ) then begin //Verifica se o cursor atende aos valores dos campos chaves
			if Assigned( Self.FOnCheckedRecord ) then begin
				...
				Checked:=Str_Pas.StrToBool(Request.ContentFields.Values[ Self.WebGridName + '.' + IntToStr( i+1 ) ]); //Falou em WebFields +1 no valor
   ...
   for i:=0 to DS.FieldCount-1 do begin
       ...
       if Request.ContentFields.IndexOfName( WFName ) <> -1 then begin
           case DS.Fields.Fields[i].DataType of
               ftFloat : begin
                   DS.Fields.Fields[i].AsFloat:=Str_Pas.StrToFloat2(Request.ContentFields.Values[ WFName ]);
               end;
               ftCurrency : begin
                   DS.Fields.Fields[i].AsCurrency:=Str_Pas.StrToCurrency(Request.ContentFields.Values[ WFName ]);
               end;
               ftBoolean : begin
...
function TWebDataSetInfoStorage.UpdateTableData(Request : TWebRequest; Response : TWebResponse; DataSetContainer : TComponent) : TWebDataSetUpdate;
//----------------------------------------------------------------------------------------------------------------------
