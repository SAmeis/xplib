...
{$ENDIF}
{$I StFLib.inc}
//Alertas removidos por necessidade urgente
...
uses
  ActiveX, FileHnd, Registry, Str_Pas, StreamHnd, Variants, StrHnd, AnsiStrings;

const
  EMPTY_CFG = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'#13 +
  // '<!DOCTYPE ConfigDocument>'#13 +
	  '<ConfigDocument>'#13 + '</ConfigDocument>'#13;
  ...
  if (Recursive) then begin
	...
	try
	  ...
	  for x := 0 to SubKeysNames.Count - 1 do begin
		Self.Import(Source, TFileHnd.ConcatPath([SourceKeyName, SubKeysNames[x]]),
...
{ -**********************************************************************
  ...
  ************************************************************************ }
{ -------------------------------------------------------------------------------------------------------------------------------- }
function TXMLBasedSettings.GetAttributeValue(const FullName: string; VarType: Word;
	ADefaultValue: TDefaultSettingValue = nil): variant;
{ {
...
begin
  ...
  try
	...
	content := AnsiStrings.AnsiStrAlloc(contentSize);
	try
	  ...
	  AnsiStrings.StrDispose(content);
	end;
  finally
	FreeMem(Buffer);
  end;
