...
{$ENDIF}
{$I StFLib.inc}
{ TODO -oroger -cfuture : Unit totalmente fora da compatibilidade unicode, perigo certo }

//tentando manter formatação abaixo
{
  ///----------------------------------------------------------
...
begin
	...
	fID3Genres.CommaText := '"Blues","Classic Rock","Country","Dance","Disco","Funk","Grunge",' +
		...
		'"Tribal","Acid Punk","Acid Jazz","Polka","Retro","Musical",' + '"Rock & Roll","Hard Rock",' //79 standard genres
		...
		'"Sonata","Symphony","Booty Bass","Primus","Porn Groove",' + '"Satire","Slow Jam","Club","Tango","Samba","Folklore",' +
		'"Ballad","Power Ballad","Rhythmic Soul","Freestyle",' + '"Duet","Punk Rock","Drum Solo","Acapella","Euro-House",' +
		...
		'"Contemporary Ch?", "Cristian Rock", "Merengue", "Salsa",' + '"Thrash Metal", "Anime", "JPop", "Synthpop", ""'; //0 .. 93h
...
var
	...
	GenreIdx, i: Integer;
	//..................................................................................................................................
	function LSRBufStr(APos, ALen: Integer): AnsiString;
	begin
		SetString(Result, nil, ALen);
		{$WARN IMMUTABLE_STRINGS OFF} {$WARN UNSAFE_CODE OFF}
		Move(b[APos], Result[1], ALen);
		{$WARN IMMUTABLE_STRINGS ON} {$WARN UNSAFE_CODE OFF}
		if AnsiStrings.AnsiPos(#0, Result) > 0 then begin
			Result := Copy(Result, 1, AnsiStrings.AnsiPos(#0, Result) - 1);
		end;
...
begin
	...
	try
		if F.Size >= ID3v1TagLen then begin
			...
			SetString(S, nil, 3);
			SetLength(st, 3);
			F.ReadBuffer(st, 3); //F.ReadBuffer(S[1], 3);
			{$WARN IMPLICIT_STRING_CAST OFF}
			S := UpperCase(PAnsiChar(st));
			{$WARN IMPLICIT_STRING_CAST ON}
		end else begin
