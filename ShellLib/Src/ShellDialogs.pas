...
function  ShowShellAboutDialog: Boolean; platform;
...
begin
  ...
  if (Assigned(Self.OnChange)) then begin
    ...
    case (NewOKState) of
      kbsdEnableOK:  PostMessage(Self.FDialogHandle, BFFM_ENABLEOK, 0, LPARAM(True));
      kbsdDisableOK: PostMessage(Self.FDialogHandle, BFFM_ENABLEOK, 0, LPARAM(False));
    end; {case}
  end; {if}
end;
...
begin
  ...
  else if (Self.RootFolder = rfDesktop) then begin
    BrowseInfo.pidlRoot := nil;
  end  {else if}
  ...
  case (Self.Filter) of
    kbsdBrowseForComputers:     BrowseInfo.ulFlags := BrowseInfo.ulFlags or BIF_BROWSEFORCOMPUTER;
    kbsdBrowseForDirectories:   BrowseInfo.ulFlags := BrowseInfo.ulFlags or BIF_RETURNONLYFSDIRS;
    kbsdBrowseForFileAncestors: BrowseInfo.ulFlags := BrowseInfo.ulFlags or BIF_RETURNFSANCESTORS;
    kbsdBrowseForPrinters:      BrowseInfo.ulFlags := BrowseInfo.ulFlags or BIF_BROWSEFORPRINTER;
  end; {case}
...
begin
  case (FormatResult) of
    kbsdFormatSucceeded:     Result := 0;
    kbsdFormatCancelled:     Result := SHFMT_CANCEL;
    kbsdDriveNotFormattable: Result := SHFMT_NOFORMAT;
    kbsdFormatError:         Result := SHFMT_ERROR;
    else                     Result := SHFMT_ERROR;
  end; {case}
end;
...
begin
  case (FormatResult) of
    SHFMT_CANCEL:   Result := kbsdFormatCancelled;
    SHFMT_NOFORMAT: Result := kbsdDriveNotFormattable;
    SHFMT_ERROR:    Result := kbsdFormatError;
    else            Result := kbsdFormatSucceeded;
  end; {case}
end;
...
begin
  case (RestartOption) of
    EWX_LOGOFF:         Result := kbsdLogoff;
    EWX_SHUTDOWN:       Result := kbsdShutdown;
    EWX_REBOOT:         Result := kbsdReboot;
    EW_RESTARTWINDOWS:  Result := kbsdRestartWindows;
    EW_REBOOTSYSTEM:    Result := kbsdRebootSystem;
    EW_EXITANDEXECAPP:  Result := kbsdExitAndExecApp;
    else                Result := kbsdRestartWindows;
  end; {case}
end;
...
begin
  case (ShellObjectType) of
    kbsdPathObject:    Result := OPF_PATHNAME;
    kbsdPrinterObject: Result := OPF_PRINTERNAME;
    else               Result := 0;
  end; {case}
end;
...
begin
  case (ShellObjectType) of
    OPF_PATHNAME:    Result := kbsdPathObject;
    OPF_PRINTERNAME: Result := kbsdPrinterObject;
    else             Result := kbsdPathObject;
  end; {case}
end;
...
begin
  case (RunFileOption) of
    kbsdNoBrowseButton:    Result := RFF_NOBROWSE;
    kbsdNoDefaultPick:     Result := RFF_NODEFAULT;
    kbsdCalculateWorkPath: Result := RFF_CALCDIRECTORY;
    kbsdNoEditLabel:       Result := RFF_NOLABEL;
    kbsdNoSeparateMemory:  Result := RFF_NOSEPARATEMEM;
    else                   Result := 0;
  end; {case}
end;
...
begin
  case (RunFileOption) of
    RFF_NOBROWSE:      Result := kbsdNoBrowseButton;
    RFF_NODEFAULT:     Result := kbsdNoDefaultPick;
    RFF_CALCDIRECTORY: Result := kbsdCalculateWorkPath;
    RFF_NOLABEL:       Result := kbsdNoEditLabel;
    RFF_NOSEPARATEMEM: Result := kbsdNoSeparateMemory;
    else               Result := kbsdNoSeparateMemory;
  end; {case}
end;
...
begin
  case (RunFileAction) of
    kbsdRun:    Result := RF_OK;
    kbsdCancel: Result := RF_CANCEL;
    kbsdRetry:  Result := RF_RETRY;
    else        Result := RF_OK;
  end; {case}
end;
...
begin
  case (RunFileAction) of
    RF_OK:     Result := kbsdRun;
    RF_CANCEL: Result := kbsdCancel;
    RF_RETRY:  Result := kbsdRetry;
    else       Result := kbsdRun;
  end; {case}
end;
...
begin
  case (NetResourceType) of
    kbsdDiskResource:  Result := RESOURCETYPE_DISK;
    kbsdPrintResource: Result := RESOURCETYPE_PRINT;
    else               Result := RESOURCETYPE_DISK;
  end; {case}
end;
...
begin
  case (NetResourceType) of
    RESOURCETYPE_DISK:  Result := kbsdDiskResource;
    RESOURCETYPE_PRINT: Result := kbsdPrintResource;
    else                Result := kbsdDiskResource;
  end; {case}
end;
