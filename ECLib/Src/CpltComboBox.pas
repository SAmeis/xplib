	...
	case message.Msg of
		WM_KEYUP: begin
				...
				//(Message.WParam <> VK_DELETE) and (Message.WParam <> VK_SHIFT) and
					not(message.WParam in [VK_BACK, VK_CLEAR, VK_FINAL, VK_PRIOR, VK_NEXT, VK_END, VK_HOME, VK_LEFT, VK_RIGHT,
					VK_SELECT, VK_DELETE, VK_SHIFT, VK_RETURN, VK_TAB]) and (FTextCompletion = TRUE) then begin
					//Use CB_FINDSTRING to locate the string in the Items property
					rc := Perform(CB_FINDSTRING, WParam(-1), Integer(PChar(Caption)));
