			...
			if ActualLen > Width then begin
				...
				if Len <> 0 then begin
					...
					if ActualLen > Width then begin
						...
						Result := Result + TruncTextCanvasWidthRigth(Canvas, Copy(Text, Len + 1, Length(Text)), Width - ActualLen);
					end;
				end else begin
					Result := EmptyStr;
				end;
...
begin
	...
	if (Flags and DT_CALCRECT <> 0) and ((DspTxt = EmptyStr) or ShowAccelChar and (DspTxt[1] = '&') and (DspTxt[2] = #0)) then begin
		DspTxt := DspTxt + ' ';
	end;
