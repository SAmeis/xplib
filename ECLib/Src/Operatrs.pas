...
type
	...
	public
		...
		property MinOperandNumber : integer read GetMinOperandNumber;
		property Operator : TVisualOperator read GetOperator write SetOperator;
	...
	public
		...
		constructor Create(const AOperatorName : string; AOperatorDictionary : TOperatorDictionary;
...
var
	Operator : TVisualOperator;
begin
	//Limpa elementos anteriores
	Self.Clear;

	//Cria o operador nulo
	Operator := TVisualOperator.Create(Self.OperatorList);
	Operator.DisplayName := 'Operando';
	Operator.FOperatorType := otUnknown;
	Operator.FOperatorText := EmptyStr;
	Operator.FMinOperandNumber := 0;
	Operator.FMaxOperandNumber := 32;
	Operator.BackGroundColor := clYellow;
	Operator.IconString := '?';
	Operator.OperatorArgumentType := [ortUnknown];
	Operator.FOperatorReturnType := [ortUnknown];
	Self.AddIconImage(Operator);
	Self.UpdateGUI(Operator);

	//Cria o operador Resultado
	Operator := TVisualOperator.Create(Self.OperatorList);
	Operator.DisplayName := 'Resultado';
	Operator.BackGroundColor := clNavy;
	Operator.FOperatorType := otResult;
	Operator.FOperatorText := EmptyStr;
	Operator.FMinOperandNumber := 0;
	Operator.FMaxOperandNumber := 32;
	Operator.IconString := '=';
	Operator.FOperatorArgumentType := [ortUnknown];
	Operator.FOperatorReturnType := [ortUnknown];
	Self.AddIconImage(Operator);
	Self.UpdateGUI(Operator);
	...
	Operator := TVisualOperator.Create(Self.OperatorList);
	Operator.DisplayName := 'Valor';
	Operator.BackGroundColor := clGray;
	Operator.FOperatorType := otVariable;
	Operator.FOperatorText := EmptyStr;
	Operator.FMinOperandNumber := 0;
	Operator.FMaxOperandNumber := 0;
	Operator.IconString := 'X';
	Operator.FOperatorArgumentType := [ortUnknown];
	Operator.FOperatorReturnType := [ortUnknown];
	Self.AddIconImage(Operator);
	Self.UpdateGUI(Operator);
...
{$TYPEDADDRESS OFF}
	if (ortUnknown in ArgumentType) or (ortUnknown in Self.OperatorArgumentType) then begin
		...
		rt1 := @Self.OperatorArgumentType; //Tipos dos argumentos
...
begin
	...
	Self.Add(SubOperand);
	Self.CheckArguments(); //Procura remover elementos inconsistentes
end;
...
begin
	case Self.Operator.FOperatorType of
		...
		otOperator	: begin
			{ TODO -oRoger -cDSG : Arrumar forma de montar todos os operadores }
			Result := EmptyStr;
			for i := 0 to Self.Count - 1 do begin
...
begin
	...
	end else begin  //Se nil para valor o tipo sera otUnknow
...
//----------------------------------------------------------------------------------------------------------------------
begin
	if Self.Operator.OperatorType in [otVariable, otConstant] then begin //Altera apenas para os tipos mutaveis
		FValue := Value;
	end;
end;
