...
begin
    ...
    Hnd := CreateFile(PChar(Self.CanonicalPathname), GENERIC_WRITE, 0, nil, OPEN_EXISTING,
    ...
    if (FindFirst(TFileHnd.ConcatPath([DirName, '*.*']), faAnyFile, SR) = ERROR_SUCCESS) then begin
        try
            repeat
                ...
                if (Self.IsRecursive and ((SR.Attr and faDirectory) = faDirectory) and (SR.Name <> '.') and (SR.Name <> '..'))
                then begin
