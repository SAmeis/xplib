...
const
  ...
  INVALID_FILENAME_CHARS_HIGH_CRITIC: TSysCharSet = [#0 .. #32] + [#128 .. #255] + ['*', '?', '"', '\', '/', '<', '>', #9 { tab } ,
	  ':', '{', '}', '[', ']', '¦'] { INVALID_FILENAME_CANONIC_CHARS } + ['-', ' ' { espaco } , '&'] { INVALID_FILENAME_LINUX_EXT };
...
begin
  ...
  repeat
	...
	try
	  if Found then begin
		if not MoveFileEx(PChar(TFileHnd.ConcatPath([Path, SR.Name])), PChar(TFileHnd.ConcatPath([DestDir, SR.Name])), Flags) then
		begin
		  Result := GetLastError();
		end;
	  end;
