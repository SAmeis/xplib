...
const
  ...
  VISUAL_CHARS_LIST = ['\', '$', '%', '&', '@', '*', '+', '/', 'A' .. 'Z', 'a' .. 'z', '{', '}', '(', ')', '>', '<', '?',
	  '0' .. '9', '[', ']'];
  INTEGER_CHARS_LIST              = ['0' .. '9', '-', '+'];
  FLOAT_CHARS_LIST                = INTEGER_CHARS_LIST + [',', '.', 'E', 'e'];
  NOT_ALPHA_LOWER_CASE_CHARS_LIST = [#0 .. #255] - [#9, #32, 'a' .. 'z', 'á', 'é', 'í', 'ó', 'ú', 'à', 'ã', 'õ', 'â', 'ê', 'ô',
	  'ç', 'ü'];
...
function ASCIISearchString(const NormalSrcString: string): string; deprecated 'Use TStrHnd.ASCIISearchString()';
...
function RepChar(Caracter: char; Many: cardinal): string; deprecated 'Use StringOfChar()';
...
begin
	Result := TStrHnd.ASCIISearchString( NormalSrcString );
end;
...
begin
	Result:=StringOfChar( Caracter, Many );
end;
