...
const
  ...
  MonthOrdinalNames: array [meJan .. meDec] of string = ('Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho',
...
class procedure TTimeHnd.ExtractMask(const Format, S: string; Ch: char; Cnt: Integer; var I: Integer; Blank, Default: Integer);
var
  Tmp: string;
  J, L: Integer;
begin
...
begin
  ...
  if not(ScanNumber(S, MaxInt, Pos, N1) and ScanChar(S, Pos, FS.DateSeparator) and ScanNumber(S, MaxInt, Pos, N2)) then begin
	Exit;
  end;
  ...
  if SysLocale.FarEast and (System.Pos('ddd', FS.ShortDateFormat) <> 0) then begin { ignore trailing text }
	...
	else { stop at time prefix } begin
	  repeat
		while (Pos <= Length(S)) and (S[Pos] <> ' ') do begin
		  Inc(Pos);
		end;
		ScanBlanks(S, Pos);
	  until (Pos > Length(S)) or (AnsiCompareText(FS.TimeAMString, Copy(S, Pos, Length(FS.TimeAMString))) = 0) or
...
begin
  ...
  I := Pos;
  N := 0;
  while (I <= Length(S)) and (longint(I - Pos) < MaxLength) and CharInSet(S[I], ['0' .. '9']) and (N < 1000) do begin
