...
{ {
  Copia valor de Name da chave atual para uma outra de mesmo nom em DestKey. Esta documentação necessita ser validada posteriormente
...
begin
  ...
  if Len > 0 then begin
	...
	Buffer := System.AllocMem(Len); //necessita de buffer zerado
	try
	  ...
	  for I := 0 to List.Count - 1 do begin
		if Self.OpenKey('\' + TFileHnd.ConcatPath([BaseSrc, List.Strings[I]]), False) then begin
		  if RegDest.OpenKey('\' + TFileHnd.ConcatPath([BaseDest, List.Strings[I]]), True) then begin
			...
			raise ERegistryException.CreateFmt('Falha criando chave:'#13'"%s"', [TFileHnd.ConcatPath([BaseDest, List.Strings[I]])]);
		  end;
		end else begin
...
begin
  ...
  Self.Access := KEY_QUERY_VALUE or KEY_WOW64_64KEY;
  { TODO -oroger -clib : avaliar a inclusão de KEY_WOW64_KEY (para aplicativos 32) ao lado }
  {$ENDIF}
  if OpenFullKey(ExtractFilePath(FullKeyValueName), False) then begin
	name := ExtractFileName(FullKeyValueName);
	if (Self.ValueExists(name)) then begin
...
/// <summary>
/// Escreve tipo de dado para multiplas linhas
/// </summary>
/// Modified by roger 28/09/2014 18:52:23 -  Alterada tipo de chamada para acalmar compilador, rotina necessita de validação
var
  Buffer: array of Char;
  Line: string;
  I, size: Integer;
begin
  {$MESSAGE WARN 'Esta rotina necessita de validação'}
  Result := False;
  size := 0;
  Line := EmptyStr;
  ...
  for I := 0 to MultiSZValues.Count - 1 do begin
	Line := MultiSZValues[I];
	{$WARN UNSAFE_CODE OFF}
	// Modified by roger 28/09/2014 - MoveChars(Line[1], Buffer[size], Length(Line)) pela linha abaixo para calar alerta de string imutablt
	MoveChars(PChar(Line)^, Buffer[size], Length(Line));
	{$WARN UNSAFE_CODE ON}
