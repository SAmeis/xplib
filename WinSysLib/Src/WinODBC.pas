...
begin
	...
	if (hLib <> 0) then begin
		...
		if (@pFn <> nil) then begin
			{$WARN EXPLICIT_STRING_CAST_LOSS OFF}  {$WARN IMPLICIT_STRING_CAST_LOSS OFF}
			strAttr := Format('DSN=%s' + #0 + 'Server=%s' + #0 + 'Description=%s' + #0 + 'DATABASE=%s' + #0 +
				'Trusted_Connection=%s' + #0 + #0, [cDSNName, cServer, cDescription, cDataBase, cLogon]);
			fResult := pFn(0, ODBC_ADD_DSN, PAnsiChar(AnsiString(cDriver)), PAnsiChar(strAttr));
			{$WARN EXPLICIT_STRING_CAST_LOSS ON}  {$WARN IMPLICIT_STRING_CAST_LOSS ON }
			...
			raise exception.Create('O Sistema não pode Carregar a Biblioteca ODBCCP32.DLL.' + #13 +
				'Por favor Contacte o Desenvolvedor.');
		end;
	end;
	{$TYPEDADDRESS ON}
end;
