...
begin
	...
	//ALERTA: Pode-se pensar em colocar ParamStr(0) de modo a isolar apenas o aplicativo
...
var
	...
	begin
		...
		for I := 0 to Tokens.Count - 2 do begin
			...
			if (AnsiCompareText(ModName, ModName_Shift) = 0) or (AnsiCompareText(ModName, LocalModName_Shift) = 0) then begin
				M := M or MOD_SHIFT;
			end else if (AnsiCompareText(ModName, ModName_Ctrl) = 0) or (AnsiCompareText(ModName, LocalModName_Ctrl) = 0) then begin
				M := M or MOD_CONTROL;
			end else if (AnsiCompareText(ModName, ModName_Alt) = 0) or (AnsiCompareText(ModName, LocalModName_Alt) = 0) then begin
				M := M or MOD_ALT;
			end else if (AnsiCompareText(ModName, ModName_Win) = 0) or (AnsiCompareText(ModName, LocalModName_Win) = 0) then begin
				M := M or MOD_WIN;
			end else begin
	...
	begin
		...
		if Tokens.Count > 0 then begin
			...
			if Length(KeyName) = 1 then begin
				...
				if (KeyName <> ModName_Ctrl) and (KeyName <> LocalModName_Ctrl) and (KeyName <> ModName_Alt) and
					(KeyName <> LocalModName_Alt) and (KeyName <> ModName_Shift) and (KeyName <> LocalModName_Shift) and
