...
Begin
	...
	If Not Enabled Then	Begin
		...
		Exit;
	End;
	If NameForMapFile <> '' Then begin
		FMapFileName := NameForMapFile;
	end;
	If NameForMutex <> '' Then begin
		FMutexName := NameForMutex;
	end;
	tail := PChar(PrepCommandTail);
	If Not CreateKernelObjects Then begin
		Exit;
	end;
	SharedRecPtr := MappedPagePtr;
	If Not GetMutex Then begin
		Exit;
	end;
	If InMainInstance Then Begin
		ZeroMemory(MappedPagePtr, PageSize);
		SharedRecPtr^.MainInstanceHandle := Application.Handle;
		SurrenderMutex;
		Result := True;
	End else Begin
		With SharedRecPtr^ Do Begin
			...
			HasData := True;
			If Assigned(FWriteSharedData) Then	Begin
		...
		End; // with
		SurrenderMutex;
		If RestoreMainInstance And IsIconic(MainHandle) Then Begin
			SetForegroundWindow(MainHandle);
			If Not PostMessage(MainHandle, WM_SYSCOMMAND, SC_RESTORE, 0) Then begin
