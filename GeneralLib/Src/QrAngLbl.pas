...
type
  ...
  TAngledValues = record
	fntWidth, fntHeight, txtWidth, txtHeight, gapTxtWidth, gapTxtHeight, totWidth, totHeight, posLeft, posTop, posX,
		posY: Integer end;
	...
	{$ENDIF} { DEF QR1X }
		private FAnchorStyle: TAnchorStyle;
	FAngle: Integer;
	FAutoSize: Boolean;
	FCaption: TCaption;
  ...
  private
	{$IFDEF QR2X}
	FDataSourceName: TComponentName;
	FDataSet: TDataSet;
	FDataField: string;
	FField: TField;
...
function EnumTTFontsProc(var pLogFont: TLogFont; var pTextMetric: TTextMetric; pFontType: Integer; pData: Pointer): Integer; export;
...
begin
  ...
  try
	...
	try
	  EnumFonts(DC, nil, EnumProc, Pointer(@FTTFonts));
	finally
...
procedure TQRAngledCustom.CalculateAngledValues(pCanvas: TCanvas; pCaption: TCaption; pZoom: Integer);
var
  angB: Real;
...
begin
  ...
  case FAngle of
	0 .. 89:
	  angB := DegToRad(90 - FAngle);
	90 .. 179:
	  angB := DegToRad(FAngle - 90);
	180 .. 269:
	  angB := DegToRad(270 - FAngle);
  else { 270..359 }
  ...
  if FAnchorStyle in [asNone] then begin
	FValues.posLeft := Left;
	FValues.posTop := Top;
  end else if FAnchorStyle in [asTextLeft] then begin
  ...
  end else if FAnchorStyle in [asTextRight] then begin
	{ Calculate Left position }
	case FAngle of
	  90 .. 179, 180 .. 269:
		FValues.posLeft := Left
  ...
  if (FAnchorStyle in [asTextLeft, asTextRight, asTextCenter]) and not AutoSize then
	if FAnchorStyle in [asTextLeft] then
	  ...
	  end else if FAnchorStyle in [asTextRight] then
	  case FAngle of
		0 .. 89: begin
...
begin
  ...
  with Canvas do begin
	...
	{TODO -oroger -cLib : Testar alteração de passagem para XE6 abaixo}
	//ExtTextOut(Handle, FValues.posX, FValues.posY, ETO_CLIPPED, @aRect, @FCaption[1], Length(FCaption), nil);
	ExtTextOut(Handle, FValues.posX, FValues.posY, ETO_CLIPPED, @aRect, PChar(FCaption), Length(FCaption), nil);
  end;
end;
...
begin
  OldCaption := Caption;
  NewCaption := Caption;
  if Assigned(FOnPrintEvent) and not(csDesigning in ComponentState) then begin
...
begin
  if FAnchorStyle <> pValue then begin
	FAnchorStyle := pValue;
	AdjustSizes;
	Invalidate { Redraw };
  end;
...
begin
  if FAngle <> (pValue mod 360) then begin
...
var
  Message: TMessage;
begin
  if FTrueTypeAlert <> pValue then begin
...
begin
  ...
  if Assigned(FField) then
	try
	  ...
	  else if (FField is TNumericField) then
