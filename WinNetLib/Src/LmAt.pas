...
//Do we exec programs for this job periodically (/EVERY switch) or one time (/NEXT switch).

const
	{$EXTERNALSYM JOB_RUN_PERIODICALLY}
	JOB_RUN_PERIODICALLY = $01; //set if EVERY

	//Was there an error last time we tried to exec a program on behalf of this job.
...
function NetScheduleJobDel(Servername: LPCWSTR; MinJobId: DWORD; MaxJobId: DWORD): NET_API_STATUS; stdcall;

{$EXTERNALSYM NetScheduleJobEnum}
function NetScheduleJobEnum(Servername: LPCWSTR; PointerToBuffer: Pointer; PrefferedMaximumLength: DWORD; var EntriesRead: DWORD;
	var TotalEntries: DWORD; ResumeHandle: PDWORD): NET_API_STATUS; stdcall;

{$EXTERNALSYM NetScheduleJobGetInfo}
function NetScheduleJobGetInfo(Servername: LPCWSTR; JobId: DWORD; PointerToBuffer: Pointer): NET_API_STATUS; stdcall;
