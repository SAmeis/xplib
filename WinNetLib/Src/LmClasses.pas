...
{$ENDIF}
{$I WinNetLib.inc}
//REmovendo os alertas por atacado
...
{$ENDIF}
	...
	uses Windows, Classes, SysUtils, Contnrs, LmCons, LmErr;
...
begin
	if InternalCheck(LmShare.NetSessionEnum(PWideChar(ServerNameW), nil, nil, 2, Buf, MAX_PREFERRED_LENGTH, EntriesRead, TotalAvail,
		nil)) then begin
		TempBuf := Buf;
...
var
	...
	Item                   : TNetSessionItem;
begin
	if InternalCheck(SvrApi.NetSessionEnum(PChar(ServerNameA), 50, @SessionInfo, Sizeof(SessionInfo), EntriesRead, TotalAvail)) then
	begin
...
begin
	...
	with FBaseList do begin
		Result := InternalCheck(SvrApi.NetShareSetInfo(ServerNameA, PChar(ShareName), 50, @ShareInfo, Sizeof(ShareInfo),
			PARMNUM_ALL));
	end;
end;
...
begin
	...
	if InternalCheck(SvrApi.NetConnectionEnum(ServerNameA, PChar(FQualifier), 50, @ConnectionInfo, Sizeof(ConnectionInfo),
		EntriesRead, TotalAvail)) then begin
		for I                    := 0 to EntriesRead - 1 do begin
			Item                 := TNetConnectionItem.Create(Self);
