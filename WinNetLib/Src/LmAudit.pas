...
function NetAuditRead(server: LPCWSTR; service: LPCWSTR; auditloghandle: PHLog; offset: DWORD; reserved1: PDWORD; reserved2: DWORD;
	offsetflag: DWORD; bufptr: Pointer; prefmaxlen: DWORD; var bytesread: DWORD; var totalavailable: DWORD)
	: NET_API_STATUS; stdcall;
{$EXTERNALSYM NetAuditRead}
function NetAuditWrite(type_: DWORD; buf: Pointer; numbytes: DWORD; service: LPCWSTR; reserved: Pointer): NET_API_STATUS; stdcall;
...
type
	...
	_AE_LOCKOUT = record
		...
		ae_lk_username: DWORD;     //Ptr to username of client (NULL if same as computername).
		ae_lk_action: DWORD;       //Action taken on account: 0 means locked out, 1 means not.
		ae_lk_bad_pw_count: DWORD; //Bad password count at the time of lockout.
	end;
