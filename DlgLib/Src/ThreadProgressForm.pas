...
begin
	if (Self.FExecutingThread.Terminated) then begin
		...
		while (not Self.FExecutingThread.Suspended) do begin //tenta suspender tanta vezes quantas necessárias(modo deprecated)
		...
		if (ret = mrYes) then begin
			...
			while (Self.FExecutingThread.Suspended) do begin
				Self.FExecutingThread.Start(); //Modified by roger 09/10/2014 14:42:02 - .Resume() -> .Start()
			end;
		end;
	end;
end;
...
{ 1 Descendente de TThread capaz de gerar dialogo de acompanhamento de progresso }
begin
	Self.Start(); //Modified by roger 09/10/2014 14:42:02 - .Resume() -> .Start()
end;
...
{ -------------------------------------------------------------------------------------------------------------------------------- }
procedure TProgressThread.Start; //Modified by roger 09/10/2014 14:42:02 - .Resume() -> .Start()
{ {
...
begin
	if ((Self.FProgressForm <> nil) and (not Self.FProgressForm.Visible) and (GetCurrentThreadId() <> Self.ThreadID)) then begin
