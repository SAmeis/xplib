	...
	try
		...
		for i      := 0 to Self.FolderList.Count - 1 do begin
			...
			for j        := 0 to FLst.Count - 1 do begin
				...
				Self.LogDList.Add(TFileHnd.ConcatPath([Self.RootDestDir, System.Copy(FLst.Strings[j], SrcPrefixLen + 1,
					Length(FLst.Strings[j]))]));
			end;
		end;
	finally
		FLst.Free;
...
begin
	{$WARN SYMBOL_PLATFORM OFF} {$WARN UNSAFE_TYPE OFF} {$WARN UNSAFE_CAST OFF}
	//Sossega a bacurinha do compilador com bobagens iniciando abaixo
	SourceDate    := 0;
	SourceAttribs := 0;
				...
				try
					...
					on E: Exception do begin
						if Assigned(Self.FOnSourceOpenFail) then begin
							...
							Self.ErrorLog.Add(Format('Arquivo %s não pode ser acessado corretamente.'#13'%s', [SFName, E.Message]));
							Exit;
						end;
					end;
				end;
		...
		while RetryDir do begin
			...
			if not ForceDirectories(ExtractFilePath(DFName)) then begin //Forcar o caminho
				if Assigned(Self.FOnDestFolderFail) then begin
					...
					Self.ErrorLog.Add(Format('Impossível criar caminho %s'#13'%s',
						[ExtractFilePath(DFName), SysErrorMessage(GetLastError)]));
				end;
			end;
		end;
	...
	{$WARN SYMBOL_PLATFORM OFF} {$WARN UNSAFE_TYPE ON} {$WARN UNSAFE_CAST ON}
end;
...
begin
	...
	Self.CopyThread.Start(); //trocado o .Resume() original
end;
